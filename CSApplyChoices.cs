...
namespace zCulturedStart
{
    class CSApplyChoices : CSCharCreationOption
    {
        ...
        public static void CSSetEquip(Hero hero, int tier)
        {
            ...
            if (IdealTroop == null) //Fall back if idea troop logic fails
            {
                IdealTroop = (from y in CharacterObject.All
                              where y.Tier == tier && y.Culture.StringId == hero.Culture.StringId && !y.IsHero && y.Equipment.GetHumanBodyArmorSum() > 0
        ...
        public static void CSCreateKingdom()
        {
            ...
            kingdom.InitializeKingdom(textObject, textObject, Clan.PlayerClan.Culture, Clan.PlayerClan.Banner, Clan.PlayerClan.Color, Clan.PlayerClan.Color2, CSOptionSettlement(), null, null, null);
        ...
        private static void CSAddCompanionAsArmy(int NoToAdd)
        {
            ...
            for (int i = 0; i < NoToAdd; i++)
            {
                ...
                CharacterObject wanderer = (from x in CharacterObject.All
        ...
        {
            Hero mainhero = Hero.MainHero;
            Hero lord = Hero.FindAll((Hero tmp) => (tmp.Culture.StringId == mainhero.Culture.StringId) && tmp.IsAlive && tmp.MapFaction != null && !tmp.MapFaction.IsMinorFaction && tmp.IsPartyLeader && !tmp.PartyBelongedTo.IsHolding).GetRandomElementInefficiently<Hero>();
